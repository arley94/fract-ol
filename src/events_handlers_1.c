...
/*   Updated: 2024/02/23 08:58:21 by acoto-gu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_handler_1(int button, int x, int y, void *param)
{
	...
	else if (button == LEFT_CLICK && f->set == MANDELBROT && f->julia)
	{
...
int	keyboard_handler_1(int key_code, void *param)
{
	...
	if (key_code == ESCAPE_KEY_CODE)
	{
		...
		re_draw_move(key_code, f);
	else if (key_code == S_KEY_CODE)
		re_draw_shift_pallette(f);
	else if (key_code == C_KEY_CODE)
		re_draw_interpollated_pallete(f);
	else if (key_code == R_KEY_CODE)
		re_draw_random_pallete(f);
	else if (key_code == SPACE_KEY_CODE)
		re_draw_change_fractal(f);
	return (0);
}
